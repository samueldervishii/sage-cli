...
router.post("/initialize", async (req, res, next) => {
  try {
    console.log("[Chat] Initialize request received");
    ...
    console.log("[Chat] Chat service initialized successfully");
    res.json({
      success: true,
    ...
    console.error("[Chat] Error in initialization:", error.message);
    next(error);
  }
});
...
router.post("/send", async (req, res, next) => {
  try {
    ...
    console.log("[Chat] Getting chat service for session:", req.sessionId);
    const chatService = getChatService(req.sessionId);

    // Check if chat service is initialized
    if (!chatService.model) {
      console.log("[Chat] Chat service not initialized");
      return res.status(400).json({
    ...
    console.log("[Chat] Sending message to chat service...");
