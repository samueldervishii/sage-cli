...
class SimpleChat {
  ...
  setupSystemPrompt() {
    const systemPrompt = `You are Sage, an intelligent AI assistant created by Samuel. You are helpful, creative, and conversational.
...
`)
  ...
  async sendMessage(userInput) {
    ...
    if (FilesystemService.detectFileIntent(userInput)) {
      ...
      if (command) {
        try {
          // Connect to terminal service if not already connected
...
Please provide a comprehensive answer based on this information.`;
    ...
    try {
      ...
      console.log(chalk.magenta("Sage:"), chalk.white(reply));
      console.log();
      ...
      // Handle service overload errors with user-friendly message
      if (
        ...
        console.error(
          chalk.yellow(
            "Sorry for the inconvenience! The model is currently overloaded. Please try again in a few moments."
          )
        );
