...
class FilesystemService {
  ...
  isPathSafe(targetPath) {
    try {
      ...
      const dangerousPatterns = [
        ...
        /\.(bat|cmd|com|exe|scr|pif|reg|vbs|vbe|js|jar|wsf|wsh)$/i, // dangerous executables
        /^\\\\/, // UNC paths
        /(aux|con|prn|nul|lpt[1-9]|com[1-9])$/i, // Windows reserved names
        /[<>:"|?*\x00-\x1f\x7f]/g, // invalid filename characters
      ];
      ...
      // Enhanced executable file restrictions
      ...
      if (executableExtensions.test(resolvedPath)) {
        ...
        // Additional check for system executable directories
      ...
      try {
        ...
        if (stats.isSymbolicLink()) {
          ...
          // Recursively check if symlink target is safe
  ...
  async connect() {
    ...
    try {
      ...
      this.transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          // Try to suppress MCP server debug output
          MCP_LOG_LEVEL: "error",
          NODE_ENV: "production",
        },
      });
  ...
  async listDirectory(dirPath, options = {}) {
    ...
    try {
      ...
      spinner.fail(`Failed to list directory: ${error.message}`);
      throw error;
    }
  }
